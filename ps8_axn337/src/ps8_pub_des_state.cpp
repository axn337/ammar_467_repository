...
PS8DesStatePublisher::PS8DesStatePublisher(ros::NodeHandle& nh) : nh_(nh) {
    //as_(nh, "pub_des_state_server", boost::bind(&PS8DesStatePublisher::executeCB, this, _1),false) {
    ...
    g_lidar_alarm = false;
    e_stop_alarm = false; // Might be redundant, modified for lab 8
    on_alarm = false;
    e_stop_on = false; // Might be redundant, modified for lab 8
...
void PS8DesStatePublisher::initializeServices() {
    ROS_INFO("Initializing Services");
    estop_service_ = nh_.advertiseService("estop_service",
            &PS8DesStatePublisher::estopServiceCallback, this);
    estop_clear_service_ = nh_.advertiseService("clear_estop_service",
            &PS8DesStatePublisher::clearEstopServiceCallback, this);
    flush_path_queue_ = nh_.advertiseService("flush_path_queue_service",
            &PS8DesStatePublisher::flushPathQueueCB, this);
    append_path_ = nh_.advertiseService("append_path_queue_service",
            &PS8DesStatePublisher::appendPathQueueCB, this);
    
}

void PS8DesStatePublisher::initializeSubscribers() {
    alarm_subscriber_ = nh_.subscribe("/ps8_lidar_alarm", 1, &PS8DesStatePublisher::alarmCB, this); // Modified by Jonathan
    //dist_subscriber_ = nh_.subscribe("lidar_dist", 1, &PS8DesStatePublisher::alarmCB, this); // Modified by Jonathan
    ESTOP_ = nh_.subscribe("/ESTOP", 1, &PS8DesStatePublisher::estopCB, this); // Modified by Jonathan
}

//member helper function to set up publishers;

void PS8DesStatePublisher::initializePublishers() {
...
void PS8DesStatePublisher::alarmCB(const std_msgs::Bool& alarm_msg) 
{ 
...
// Code modified for lab 8

void PS8DesStatePublisher::estopCB(const std_msgs::Bool& estop_msg) 
{ 
...
bool PS8DesStatePublisher::estopServiceCallback(std_srvs::TriggerRequest& request, std_srvs::TriggerResponse& response) {
ROS_INFO("Estop trigger service callback");
	on_alarm = true; // 
	e_stop_trigger_ = true; // 
}

bool PS8DesStatePublisher::clearEstopServiceCallback(std_srvs::TriggerRequest& request, std_srvs::TriggerResponse& response) {
ROS_INFO("Estop reset service callback");
   on_alarm = false; 
  e_stop_reset_ = true; 
}

// Code modified for lab 8 stopped

bool PS8DesStatePublisher::flushPathQueueCB(std_srvs::TriggerRequest& request, std_srvs::TriggerResponse& response) {
...
bool PS8DesStatePublisher::appendPathQueueCB(ps8_axn337::pathRequest& request, ps8_axn337::pathResponse& response) {
...
void PS8DesStatePublisher::set_init_pose(double x, double y, double psi) {
...
void PS8DesStatePublisher::pub_next_state() {
