...
int main ( int argc , char ** argv ) {
	...
	ros::Rate naptime(1); // set the sleep timer for 1 Hz repetition rate
	...
	while (ros::ok())
	{
		...
		// except one core to e 100% dedicated(wastefully) to this small task
		input_float.data= input_float.data + 0.001; // increment by 0.001 each iteration
		my_publisher_object.publish(input_float); // publish the value of type Float 64 --
		// to the topic "topic1"
		naptime.sleep();
