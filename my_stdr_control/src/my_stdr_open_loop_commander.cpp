...
int main(int argc, char **argv) {
    ...
    timer=0.0; //reset the timer
    while(timer<2.25) { //spin more than 60 degrees
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=0.0; //stop moving forward
    twist_cmd.angular.z=-yaw_rate; // start spinning clockwise
    timer=0.0; //reset the timer
    while(timer<2) { //spin 60 dergees 
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
